...
export default function Biography() {
    ...
    useEffect(() => {
        if (data.length > 0) {
            ...
            data.forEach(item => {
                const decade = getDecadeYear(item.date[locale]);
                if (decade !== null) decadesSet.add(decade);
            });

            setYears(Array.from(decadesSet).sort((a, b) => a - b));
        }
    }, [data, locale]);
            ...
            (entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        ...
                        const visibleYear = getDecadeYear(entry.target.id);
                    ...
                    <div className="biography container mx-auto pt-[80px] xl:pt-[100px] px-[20px] xl:px-0">
                        ...
                        <div className="xl:grid grid-cols-12">
                                        ...
                                        {years?.map((year, index) =>
                                            <Link
                                                ...
                                                className={classNames("w-[100px] 2xl:w-[135px] block text-[15px] 2xl:text-[18px] px-[8px] 2xl:px-[17px] py-[3px] text-white mb-[6px] cursor-pointer border-blue transition-all duration-300 ease-in-out", {
                                                    "bg-white text-blue border": activeYear === year || activeElement === `event-${year}`,
                                                    "bg-blue border": year && activeYear !== year && activeElement !== `event-${year}`
                                                })}
                                            >
                                        ...
                                        {years?.map((year, index) =>
                                            <Link
                                                ...
                                                className={classNames("block text-[14px] 2xl:text-[18px] px-[10px] 2xl:px-[17px] py-[2px] xl:py-[3px] text-white cursor-pointer transition-all duration-300 ease-in-out", {
                                                    "bg-white text-blue first:rounded-bl-[5px]": activeYear === year || activeElement === `event-${year}`,
                                                    "bg-blue": activeYear !== year && activeElement !== `event-${year}`
                                                })}
                                            >
                                                <span className="block text-center">{year}</span>
                                            </Link>
                            ...
                            <div className="events col-span-12 lg:col-span-10 relative before:absolute before:left-[-82px] before:top-0 before:w-[2px] before:xl:h-full before:bg-[#4100FC]">
                                {(() => {
                                    let addedDecades = new Set();
                                    return data?.map(item => {
                                        const decade = getDecadeYear(item.date[locale]);
                                        const isFirstOfDecade = !addedDecades.has(decade);
                                        if (isFirstOfDecade) addedDecades.add(decade);

                                        const Wrapper = isFirstOfDecade ? Element : "div";
                                        const wrapperProps = isFirstOfDecade
                                            ? { name: `event-${decade}`, id: `event-${decade}` }
                                            : {};
                                            ...
                                            >
                                                <div className="hidden xl:block absolute -left-[99px] w-[36px] h-[36px] bg-blue rounded-full">
                                                    <div className={classNames("w-[10px] h-[10px] rounded-full absolute top-[50%] left-[50%] -translate-x-[50%] -translate-y-[50%]", {
                                                        "bg-white": activeElement === `event-${decade}`
                                                    })}></div>
                                                </div>

                                                {item?.date && locale &&
                                                    <span className="special-elite-regular text-[#4100FC] text-[20px] lg:text-[26px]">
                                                        {item.date[locale]}
                                                    </span>
                                                }
                                                ...
                                                <div className="md:flex md:flex-wrap gap-3">
                                                    {item?.documents?.map(document =>
                                                        ...
                                                        >
                                                            <img
                                                                className="h-[200px] md:h-[220px] md:max-h-auto max-h-[350px] object-contain"
...
export function getDecadeYear(date) {
    if (!date) return null;
