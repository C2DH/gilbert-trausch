...
export default function Chapter() {
  ...
  useEffect(() => {
    ...
    if (
      parseInt(searchParams.get('index')) === 0 ||
      ...
      .then((data) => {
        ...
        setSlideHeaders(
          data?.data?.slides
            ...
            ?.filter(({ slide }) => slide.slidable.type === 'SlideHeader'),
        )
        setIsLoading(true)
      })
      .catch((error) =>
  ...
  useEffect(() => {
    if (data?.slides && data.slides[activeIndex]) {
      setColorElement(
        data.slides[activeIndex].slidable.color_menu === '#ffffff'
          ? data.slides[activeIndex].slidable.color_menu
          : '#4100FC',
      )
  ...
  const strokeDashoffset =
    circumference - (currentInGroupIndex / currentGroupTotal) * circumference
  ...
  const handleNextClick = () => {
    ...
    if (activeIndex === 0 && firstClick) {
      ...
      navigate(
        location.pathname +
          '?index=' +
          Math.min(data.slides.length - 1, activeIndex + 1),
      )
    }
  }
        ...
        <>
            ...
            {data?.slides?.map((slide, index) => {
              return (
                index === activeIndex && (
                  ...
                  >
                    <>
                      {slide.slidable.type === 'SlideHeader' && (
                        <SlideHeader
                          data={slide}
                      ...
                      {slide.slidable.type === 'SlideMediaFull' && (
                        <SlideMediaFull data={slide} locale={locale} />
                      )}
                      {slide.slidable.type === 'SlideCitation' && (
                        <SlideCitation data={slide} locale={locale} />
                      )}
                      {slide.slidable.type === 'SlideCentralText' && (
                        <SlideCentralText data={slide} locale={locale} />
                      )}
                      {slide.slidable.type === 'SlideColumn' && (
                        <SlideColumn data={slide} locale={locale} />
                      )}
                      {slide.slidable.type === 'SlideSlider' && (
                        <SlideSlider data={slide} locale={locale} />
                      )}
                      {slide.slidable.type === 'SlideMasonry' && (
                        <SlideMasonry data={slide} locale={locale} />
                      )}
                      {slide.slidable.type === 'SlideImageText' && (
                        <SlideImageText data={slide} locale={locale} />
                      )}
                      {slide.slidable.type === 'SlideStep' && (
                        <SlideStep data={slide} locale={locale} />
                      )}
                      {slide.slidable.type === 'SlideAudio' && (
                        <SlideAudio data={slide} locale={locale} />
                      )}
                    </>
                  </motion.div>
                )
          ...
          {slideHeaders?.length > 0 && (
            <div
              ...
              className={classNames(
                'fixed xl:absolute right-[20px] z-[900] cursor-pointer transition-all duration-500',
                {
                  'top-[4px]': !isLarge,
            ...
            >
              ...
              >
                <circle
                  ...
                  cy='25'
                  r='25'
                  fill={!isOpenMenu ? colorElement : 'white'}
                />
                <path
            ...
            {data.slides[activeIndex] &&
              data.slides[activeIndex].slidable.type !== 'SlideHeader' && (
                ...
                >
                  ...
                  <span
                    className='absolute text-[16px] top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 transition-all duration-500'
                    style={{ color: colorElement }}
                  >
                    {currentInGroupIndex}/{currentGroupTotal}
                  </span>
                </motion.div>
              )}
          </AnimatePresence>
          ...
          <div className='hidden xl:block absolute right-[0] top-[50%] -translate-y-[50%] z-[100]'>
            <button
              ...
              className={classNames(
                'cursor-pointer relative right-0 bottom-[5px]',
                { 'pointer-events-none opacity-30': activeIndex === 0 },
              )}
            ...
            >
              ...
              >
                ...
                <g mask='url(#mask0_93_485)'>
                  <path
                    d='M20.8906 17.2734C20.8086 17.3828 20.6992 17.4375 20.5625 17.4375C20.4805 17.4375 20.3711 17.4102 20.2891 17.3281L14.9023 12.3789L9.48828 17.3281C9.32422 17.4922 9.05078 17.4922 8.88672 17.3008C8.72266 17.1367 8.72266 16.8633 8.91406 16.6992L14.6016 11.4492C14.7656 11.2852 15.0117 11.2852 15.1758 11.4492L20.8633 16.6992C21.0547 16.8359 21.0547 17.1094 20.8906 17.2734Z'
                    fill={colorElement}
                  />
                </g>
              </svg>
            ...
            <div
              className='h-[1px] relative left-0 right-0 w-[80px]'
            ...
            <button
              ...
              className={classNames(
                'cursor-pointer relative right-0 top-[10px]',
                {
            ...
            >
              ...
              >
                ...
                <g mask='url(#mask0_93_483)'>
                  <path
                    d='M9.10938 12.7266C9.19141 12.6172 9.30078 12.5625 9.4375 12.5625C9.51953 12.5625 9.62891 12.5898 9.71094 12.6719L15.0977 17.6211L20.5117 12.6719C20.6758 12.5078 20.9492 12.5078 21.1133 12.6992C21.2773 12.8633 21.2773 13.1367 21.0859 13.3008L15.3984 18.5508C15.2344 18.7148 14.9883 18.7148 14.8242 18.5508L9.13672 13.3008C8.94531 13.1641 8.94531 12.8906 9.10938 12.7266Z'
                    fill={colorElement}
                  />
                </g>
              </svg>
            ...
            {isOpenMenu && (
              ...
              >
                ...
                <div className='relative'>
                  <div
                    className='absolute top-[12px] left-[30px] flex items-center cursor-pointer'
                  ...
                  >
                    ...
                    >
                      <path
                        d='M1.14062 5.86328L6.20312 0.800781C6.41406 0.589844 6.80078 0.589844 7.01172 0.800781C7.22266 1.01172 7.22266 1.39844 7.01172 1.60938L2.89844 5.6875H18.4375C18.7188 5.6875 19 5.96875 19 6.25C19 6.56641 18.7188 6.8125 18.4375 6.8125H2.89844L7.01172 10.9258C7.22266 11.1367 7.22266 11.5234 7.01172 11.7344C6.80078 11.9453 6.41406 11.9453 6.20312 11.7344L1.14062 6.67188C0.929688 6.46094 0.929688 6.07422 1.14062 5.86328Z'
                        fill='white'
                      />
                    </svg>
                    <span className='uppercase text-white text-[16px] font-normal pl-[10px] cursor-pointer'>
                      {t('change_chapter')}
                    </span>
                  </div>
                  <img
                    src={wallpaper_menu}
                    alt='Gilbert Trausch dans son bureau'
                    className='w-full h-[200px] lg:h-[220px] xl:h-[250px]'
                  />
                  <span className='absolute top-1/2 left-8 transform -translate-y-1/2 text-[24px] md:text-[30px] xl:text-[40px] xl:leading-[42px] text-white'>
                    {title}
                  </span>
                </div>
                  ...
                  {slideHeaders?.map((header, index) => {
                    return (
                      <h3
                        ...
                        className={classNames(
                          'text-white cursor-pointer hover:opacity-100 w-[85%] relative flex items-center gap-3 pb-[25px]',
                          {
                            "before:content-[''] before:block before:w-[1px] before:h-[30px] before:bg-white":
