        ...
        <>
            ...
            >
                <div className="container mx-auto flex flex-col justify-center h-full">
                    ...
                    {isMobile &&
                        <div className='w-full flex justify-center mb-[10px] lg:mb-[20px]'>
                    ...
                    <div className="grid grid-cols-12 px-[20px] xl:px-0">
                        <div className="col-span-12 md:col-span-10 md:col-start-2 lg:col-span-6 flex flex-col items-center lg:items-start">
                            <h1 className="font-extralight text-[24px] md:text-[36px] 2xl:text-[50px] uppercase tracking-[2px] text-[#4100FC] mb-[10px] lg:mb-[20px] text-center lg:text-left">{t('magicNotebooks')}</h1>
                            
                            <p className="text-[14px] xl:text-[22px] text-blue mb-[30px] lg:mb-[80px] text-center lg:text-left">{t('description_magicNotebooks')}</p>
                            
                            {isLoaded &&                                
                                <ul className="text-[18px] md:text-[20px] lg:text-[24px] 2xl:text-[36px] 2xl:leading-[48px] pl-0">
                                    {data?.map((item, index) => (
                                        <li key={item.id} className="group mb-[20px] lg:mb-[50px] last:mb-0 flex gap-5 font-extralight w-fit relative">
                                            <span className="transition-all duration-500 group-hover:text-[#4100FC] absolute left-0 top-0 ">{romanize(index)}.</span>
                                            <Link to={`/magic-notebook/${item.slug}`} className="pl-[40px] transition-all duration-500 group-hover:text-[#4100FC] group-hover:lg:pl-[60px]">{item.name[locale]}</Link>
                                        </li>
                                    ))}
                                </ul>
                            }
