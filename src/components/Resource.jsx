  ...
  return (
    <motion.div
      ...
      className={classNames(
        'popup_resource h-[100dvh] flex flex-col overflow-hidden  ',
        { 'fixed inset-0 z-[9999]': location.state?.modal },
      )}
    >
      <div className='lg:hidden py-[10px] border-b border-black h-[40px]'>
        <span
          className='block text-[14px] text-center cursor-pointer hover:text-[#4100FC] duration-500 uppercase font-light'
          onClick={() =>
            location.state?.modal ? navigate(-1) : navigate('/virtual-tour')
          }
        >
          {t('close')}
        </span>
        ...
        <div className='grid grid-cols-12'>
          <div className='col-span-12 lg:col-span-8 2xl:col-span-9 border-r border-black lg:h-[100dvh] relative pt-[20px] lg:pt-0 lg:flex justify-center items-center'>
            {data.type === 'image' && data.optimized_url && (
              <div className='px-[20px] w-full'>
                <ImageZoom
                  image={
                    isMobile
                      ? data.optimized_url.thumbnail.url
                      : data.optimized_url.large.url
                  }
            ...
            {data.type === 'audio' && (
              <div className='aspect-square w-2/3 md:w-1/3 flex flex-col items-center'>
                {data.cover?.[locale] ? (
                  <img
                    src={data.cover[locale]}
                    alt={data.name[locale]}
                    className='rounded-[6px] mb-[30px] max-h-[250px] md:max-h-none'
                  />
                ) : (
                  <div className='bg-[#DBDBD0] flex justify-center items-center relative mb-[30px] border border-black rounded-[6px]'>
                    <img
                      src={bgAudio}
                      alt='Logo audio'
                      className='rounded-[6px] max-h-[250px] md:max-h-none'
                    />
                  </div>
                )}
          ...
          <div className='col-span-12 lg:col-span-4 2xl:col-span-3 flex flex-col overflow-auto'>
            <div className='hidden border-b border-black pl-[20px] h-[40px] lg:flex items-center'>
              <span
                className='cursor-pointer text-[18px] font-light hover:text-[#4100FC] duration-500 uppercase'
                onClick={() =>
                  location.state?.modal ? navigate(-1) : navigate('/resources')
                }
              >
                {t('close')}
              </span>
            ...
            <div className='lg:h-[calc(100dvh-55px)] lg:overflow-auto'>
              <div className='content py-[30px] px-[20px] border-b border-black'>
                <div className='flex lg:block justify-between items-center'>
                  {data?.date &&
                    (data.display_year ? (
                      <span className='block text-[15px] mb-[25px]'>
                        {formatDateYear(data.date, locale)}
                      </span>
                    ) : (
                      <span className='block text-[15px] mb-[25px]'>
                  ...
                  {data?.type && (
                    <span className='inline-block py-[4px] px-[6px] text-[12px] leading-[12px] mb-[25px] bg-blue text-white rounded-[4px]'>
                      {formatTypeName(data.type, locale)}
                    </span>
                  )}
                </div>

                <h1 className='text-[22px] leading-[32px] font-normal mb-[15px]'>
                  {data.name[locale]}
                </h1>

                {data?.description && (
                  <div className='mb-[30px] ]'>
                    {formatRichText(data.description[locale])}
                  </div>
                )}
              ...
              <div className='tags py-[30px] px-[20px] overflow auto'>
                <div className='flex flex-wrap gap-[15px]'>
                  {data?.tags?.map((tag) => (
                    <div
                      key={tag.id}
                      className='py-[7px] px-[14px] text-[12px] leading-none uppercase border border-black rounded-[6px]'
                    >
                      {tag.name[locale]}
                    </div>
                  ))}
        ...
        <>
          <TransformComponent wrapperStyle={{ width: '100%' }}>
            <img
              src={image}
              alt={alt}
              className='lg:max-h-[calc(100dvh-120px)]'
            />
          </TransformComponent>
        ...
        <div className='flex justify-center'>
          <div className='flex cursor-pointer'>
            ...
            >
              <MagnifyingGlassMinusIcon
                style={{ width: '50px' }}
                className={classNames('px-[15px] py-[8px]', {
                  'pointer-events-none opacity-30': zoom === null || zoom === 1,
                })}
              />
            </div>
            <div
              className='uppercase text-[14px] flex items-center border-t border-b border-black px-[12px]'
            ...
            >
              <MagnifyingGlassPlusIcon
                style={{ width: '50px' }}
                className={classNames('px-[15px] py-[8px]', {
                  'pointer-events-none opacity-30': zoom === 8,
                })}
              />
            </div>
          </div>
