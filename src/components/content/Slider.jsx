...
export default function Slider({ items, locale }) {
    ...
    return (
        <>
                    ...
                    return (
                        <SwiperSlide key={item.id} className="">
                            {(item.type !== "audio" && item.optimized_url?.large) && (
                                <img src={item.optimized_url?.large.url} alt={item.name[locale]} className="w-auto h-[300px] xl:h-[400px] lg:h-auto lg:max-h-[calc(100vh-200px)]" />
                            )}
            ...
            {items?.length > 1 &&  
                <>          
                    <div className="absolute min-w-[116px] lg:bottom-[20px] translate-y-[24px] lg:translate-y-0 left-[50%] -translate-x-[50%] border border-black lg:flex px-3 rounded-[60px]">
